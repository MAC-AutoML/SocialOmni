...
def wait_server_ready(base_url: str, timeout_sec: int, interval_sec: float) -> bool:
...
def run_benchmark(
    ...
    result_file = ROOT / "results" / f"results_{model_name}_level{level}.json"
            ...
            if now >= next_tick:
                if level == 1:
                    ...
                    if processed != last_processed:
                        print(
                            f"[PROGRESS] {model_name}: processed={processed}/{max_samples} "
                        ...
                        print(
                            ...
                            f"(processed={processed}/{max_samples}, scored={total}, next_sample_id={next_pending_id})"
                        )
                else:
                    try:
                    ...
                    if processed != last_processed:
                        ...
                        print(
                            f"[PROGRESS] {model_name}: still running... "
                            f"(processed={processed}/{max_samples}, q1_correct={q1_correct}, q2_count={q2_count})"
                        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Auto-start local server and run single-model test")
    parser.add_argument("--model", required=True, help="Model name, e.g. qwen3_omni / gpt4o")
    parser.add_argument("--max-samples", type=int, default=1, help="Number of test samples, default 1")
    ...
    parser.add_argument("--keep-server", action="store_true", help="Keep server started by this script running after test")
...
def main() -> None:
    ...
    out_dir = ROOT / "results" / "smoke_tests" / f"{model_name}_{ts}"
    ...
    try:
        if is_local_model(model_name):
            ...
            if server_health_ok(endpoint.base_url):
                ...
                print("[INFO] Service is not running, starting automatically...")
        ...
        if started_by_script and server_proc and not args.keep_server:
            stop_server(server_proc)
            print("[INFO] Stopped local server started by this script.")
        elif started_by_script and server_proc and args.keep_server:
            print("[INFO] keep-server=true, leaving service running.")
