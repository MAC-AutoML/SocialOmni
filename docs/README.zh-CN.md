...
MiniCPM-o 4.5 的分类指标：Who macro-F1 **72.52**；When macro-F1 **31.87**。

Ming-Omni 2.0 的分类指标：Who macro-F1 **50.47**；When macro-F1 **47.90**。

所有指标为 0–100 标度，越高越好；— 表示该指标暂无分数。

[逐样本回答、评分与评测设置](../evaluation/results/) · [历史材料归档](../evaluation/results/archive/README.md)
