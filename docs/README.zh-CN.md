...
以下均采用 9 月 24 日的新三评委组，指标为 0–100 标度。论文原回答与托管实测的输入和分类协议不同，分别列组，不合成为总排名。[逐样本结果与设置](../evaluation/results/modern-panel-20260924/README.md)记录完整评分及格式诊断。
...
Who 和 When 使用严格标签解析；请求失败仍计入分母。格式辅助解析仅作诊断，不替换主分。

### 论文原回答重新评分

复用 688 条归档回答，Who、When 和覆盖率保留原输出。缺少每模型完整 128 条强制回答，所以本组 QGold 留空。
...
原论文评委组及此前替换评委组的结果已移至[历史材料归档](../evaluation/results/archive/README.md)，原始数据、评分、论文及复现脚本继续保留。
